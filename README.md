...
> 基于 Flask + Tkinter 的双进程桌面文件传输服务，通过 `--run-server` 参数区分 GUI 控制与 Flask 服务，主进程 0 阻塞，支持打包为单文件 exe 分发。

### 高频面试问题

#### 架构层

1. **Q：为什么 GUI 和 Flask 不写在一个进程里？**
   - A：① Flask `debug=True` 会重载，和 Tk 主线程冲突 ② GIL 下 CPU 密集阻塞 ③ GUI 关了但服务进程还能独立跑 ④ 崩溃隔离

2. **Q：条件导入 Flask 有什么好处？**
   - A：GUI 模式启动更轻量、降低依赖要求（即便没装 Flask 也能开 GUI 看到报错）

3. **Q：停止服务时做了哪些防御？**
   - A：4 层 — ① NoSuchProcess 检测 ② terminate 子进程 + wait_procs + kill ③ 主进程 terminate + 超时 kill ④ 扫描端口全局 kill

#### 后端层

4. **Q：Flask `before_request` / `after_request` 钩子如何实现访问日志？**
   - A：before 存 `start_time`，after 计算响应时间 + 组装 extra 参数写 access_logger

5. **Q：`send_from_directory` 相比 `open()` 直接返回有什么优势？**
   - A：自动处理 MIME、Range、304、`as_attachment` 文件名编码、安全路径校验

6. **Q：UUID 前缀防重名，两个用户同时上传同名文件会冲突吗？**
   - A：不会。UUID4 是 122 位随机数，碰撞概率可忽略；文件名格式为 `uuid4_原名`

#### 运维层

7. **Q：`sys.frozen` 和 `sys._MEIPASS` 是什么？**
   - A：PyInstaller 单文件打包时 frozen=True；解压临时目录路径放 _MEIPASS，templates/static 要从这里拼

8. **Q：日志为什么用 RotatingFileHandler 不用 TimedRotatingFileHandler？**
   - A：按大小轮转更适合文件传输场景（流量不均匀），按时间轮转在低峰期会产生大量空文件

9. **Q：多编码降级读取（UTF-8 → GBK → Latin-1）的意义？**
   - A：Windows 老日志可能是 GBK，全部失败时 Latin-1 是"永不抛 UnicodeDecodeError"的终极兜底

#### 前端层

10. **Q：防抖（debounce）如何实现？**
    - A：`setTimeout` + `clearTimeout`，输入停 300ms 才请求，避免每按一个键打一次接口

11. **Q：XHR `upload.progress` 相比 fetch 有什么优势？**
    - A：Fetch 标准没有上传进度流（除非手动 ReadableStream pipe），XHR 原生支持

#### 场景题

12. **Q：大文件分片上传如何改造？**
    - A：前端 `File.slice` 按 MB 分片，请求头 `Content-Range`，后端 temp 目录合并，记录 offset，MD5 校验

13. **Q：10 万文件目录 `/files` 接口卡顿怎么优化？**
    - A：SQLite 元数据表 + Watchdog 监听 FS 事件增量同步，或分页（limit/offset）
